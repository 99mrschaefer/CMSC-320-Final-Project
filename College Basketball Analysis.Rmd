...
```{r}
set.seed(1234) # set seed to make partition reproducible
...
error_rates_twoo <- error_rates_two[1:20,]
```
...
```{r dotplot}
dotplot(error~method, data=error_rates_two, ylab="Mean Prediction Error")
```

```{r rates}
lm(error~method, data=error_rates_two) %>% 
  tidy() %>%
  knitr::kable()
```
## 4. Conclusions
