...
# set sample size
smp_size <- floor(0.75 * nrow(ncaa_tab)) # obtain a number that is three quarters of the size of the data set

train_ind <- sample(seq_len(nrow(ncaa_tab)), size = smp_size) # create a vector specifying which rows will belong
# to the training set

train <- ncaa_tab[train_ind, ] # add specified entities to the training set
test <- ncaa_tab[-train_ind, ] # add all other entities to the validation set
```
...
href=https://ggplot2.tidyverse.org/>here</a>. We also utilize the vioplot library for violin plots. Documentation 
for this library can be found <a href=https://cran.r-project.org/web/packages/vioplot/index.html>here</a>.

### 2.1 Univariate analysis
First, we look at the univariate distributions. This entails observing distributions of individual attributes.

Below we look at the univariate distribution of tournament placement using a histogram:
```{r univar1}
ncaa_tab %>% 
...
qualifying games</a>, then elimination works as a factor of $TotalData*(\frac{1}{2})^r$ where r is rank.

Next we look at distribution of seed via a histogram:
```{r univar2}
ncaa_tab %>% 
  ggplot(aes(x=SEED)) + 
  geom_histogram(stat="count",fill="red")
...
Now, let's take a look at win-loss ratio, this time using a density plot:
```{r uninvar3}
ncaa_tab %>%
...
Next, we look at offensive efficiency using violin plots:
```{r univar5}
...
little to no skew. The shape and positioning of the white circle shows a clear concentration of data at the 
median. The plots also shows that teams are generally more efficient offensively relative to defensively. This  
makes sense, since the teams in our dataset performed highly relative to the rest of the nation in order to
qualify for the tournament, implying that a majority of the time, they must score more points per 100
...
We can see thqt this data is skewed negatively. As we might guess, since this is a collection of the best ranked 
teams, they data is clustered above .8, meaning that most teams in the tournament have a very good chance of 
defeating an average DI team.
...
```{r decn tree}
tree <- rpart(pstsn_rank~SEED+std_winrat+std_adjoe+std_adjde+std_barthag, data=train)
rpart.plot(tree)
```
...
```{r}
fold_indices <- cvFolds(n=nrow(ncaa_tab), K=10)
...
error_rates <- sapply(1:10, function(fold_index) {
  ...
  tree_pred <- predict(tree, newdata=test_set, type="class")
  tree_error <- mean(test_set$pstsn_rank != round(tree_pred))
  c(rand_error, tree_error)
  })
rownames(error_rates) <- c("forest", "tree")
