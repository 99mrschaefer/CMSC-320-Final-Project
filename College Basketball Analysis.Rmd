...
href=https://mode.com/blog/violin-plot-examples/>Here</a> is a helpful resource which gives a bit more detail on 
violin plots.
...
on their seed. <a href=https://www.mathsisfun.com/data/probability-events-conditional.html>This resource</a> 
...
## 4. Model Evaluation
```{r}
ncaa_tab <- subset(ncaa_tab, select = -c(ADJOE,ADJDE,BARTHAG,win_ratio))
```

```{r}
fold_indices <- cvFolds(n=nrow(ncaa_tab), K=10)

error_rates_bytwo <- sapply(1:10, function(fold_index) {
  test_indices <- which(fold_indices$which == fold_index)
  test_set <- ncaa_tab[test_indices,]
  train_set <- ncaa_tab[-test_indices,]
  
  forest = randomForest(x = train_set[-1], y = train_set$pstsn_rank,
                          ntree = 500, random_state = 0)
  forest_pred = predict(forest, newdata = test_set[-1], type="response")
  forest_error <- mean(abs(test_set$pstsn_rank - round(forest_pred))>2)
  
  tree <- rpart(pstsn_rank~SEED+std_winrat+std_adjoe+std_adjde+std_barthag, data=train_set)
  tree_pred <- predict(tree, newdata=test_set, type="vector")
  tree_error <-  mean(abs(test_set$pstsn_rank - round(tree_pred))>2)
  forest_error1 <- mean(test_set$pstsn_rank != round(forest_pred))
  tree_error1 <-  mean(test_set$pstsn_rank != round(tree_pred))
...
error_rates_bytwo <- as.data.frame(t(error_rates_bytwo))

error_rates_two <- error_rates_bytwo %>%
  mutate(fold=1:n()) %>%
  gather(method,error,-fold)
```

```{r}
error_rates_one <- error_rates_two[21:40,]
error_rates_two <- error_rates_two[1:20,]
```
